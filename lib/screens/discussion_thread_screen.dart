...
class DiscussionThreadScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final threadKey = activeSubGenreId == null
        ? null
        : Post.buildThreadKey(genreId: genre.id, subGenreId: activeSubGenreId);
    ...
    if (threadKey != null) {
      ...
      if (userProvider.user.lastThreadKey != threadKey) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!context.mounted) {
            return;
          }

          final currentUserProvider = context.read<UserProvider>();
          if (currentUserProvider.user.lastThreadKey != threadKey) {
            currentUserProvider.setLastThread(threadKey);
          }
        });
      }
    }
