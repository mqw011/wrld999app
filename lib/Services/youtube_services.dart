...
class YouTubeServices {
  ...
  Future<Map<String, List>> getMusicHome() async {
    ...
    try {
      ...
      List? result;
      try {
        result = data['contents']['twoColumnBrowseResultsRenderer']['tabs'][0]
            ['tabRenderer']['content']['richGridRenderer']['contents'] as List?;
      } catch (_) {}

      if (result == null) {
        Logger.root.severe('getMusicHome: unexpected YouTube page structure');
        return {};
      }

      List headResult = [];
      try {
        headResult = data['header']['carouselHeaderRenderer']['contents'][0]
      ...
      final List finalResult = shelfRenderer.asMap().entries.map((entry) {
        final int index = entry.key;
        final element = entry.value;
        try {
          String title = '';
          try {
            final runs = element['title']['runs'] as List?;
            if (runs != null && runs.isNotEmpty) {
              title = runs[0]['text'].toString();
            }
          } catch (_) {}
          if (title.isEmpty) title = 'Recommended ${index + 1}';

          final List? items = element['contents'] as List?;
          if (items == null) return null;
          final playlistItems = formatLockupItems(items);
          if (playlistItems.isNotEmpty) {
            return {'title': title, 'playlists': playlistItems};
          } else {
            Logger.root.severe('got empty items in getMusicHome section $index');
            return null;
          }
        } catch (e) {
          Logger.root.severe('getMusicHome element parse error: $e');
          return null;
        }
      }).whereType<Map>().toList();

      final List finalHeadResult =
          headResult.isNotEmpty ? formatHeadItems(headResult) : [];
  ...
  List formatLockupItems(List itemsList) {
    try {
      final List result = itemsList.map((item) {
        try {
          final lvm =
              item['richItemRenderer']['content']['lockupViewModel'] as Map;
          final sources = lvm['contentImage']['collectionThumbnailViewModel']
          ...
          return {
            'title': lvm['metadata']['lockupMetadataViewModel']['title']
                ['content'],
            'type': 'playlist',
            'description': lvm['metadata']['lockupMetadataViewModel']
                ['metadata']['contentMetadataViewModel']['metadataRows'][0]
                ['metadataParts'][0]['text']['content'],
            'count': '',
            'playlistId': lvm['contentId'],
            'firstItemId': lvm['contentId'],
            'image': sources.last['url'],
            'imageMedium': sources[0]['url'],
            'imageStandard':
                sources.length > 1 ? sources[1]['url'] : sources[0]['url'],
            'imageMax': sources.last['url'],
          };
        } catch (e) {
          Logger.root.severe('formatLockupItems item error: $e');
          return null;
        }
      }).whereType<Map>().toList();
      return result;
