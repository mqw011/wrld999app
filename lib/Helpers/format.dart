    ...
    try {
      ...
      data['modules'].forEach((k, v) {
        if (k.startsWith('promo') as bool) {
          final List? kList = data[k] as List?;
          if (kList != null && kList.isNotEmpty) {
            if (kList[0]['type'] == 'song' &&
                (kList[0]['mini_obj'] as bool? ?? false)) {
      ...
      for (int i = 0; i < promoList.length; i++) {
        final List? promoData = data[promoList[i]] as List?;
        if (promoData != null) {
          data[promoList[i]] = await formatSongsInList(promoData);
        }
      }
      data['collections'] = [
        'new_trending',
